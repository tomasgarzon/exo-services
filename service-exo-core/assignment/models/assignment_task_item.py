...
class AssignmentTaskItem(CreatedByMixin, TimeStampedModel):
    assignment_task = models.ForeignKey(
        'AssignmentTask', related_name='assignment_task_items',
    ...
    def mark_status(self, user_from, step, team, status):
        if not has_team_perms(
                user=user_from,
                team=team,
                perms=settings.TEAM_PERMS_FULL_VIEW_TEAM):
            raise ValidationError('{} has no permissions: TEAM_PERMS_FULL_VIEW_TEAM'.format(user_from))

        assignment_step = AssignmentStep.objects.filter(step=step).filter_by_stream(team.stream).first()
        assignment_step_team = AssignmentStepTeam.objects.filter(assignment_step=assignment_step, team=team).first()
        defaults = {
            'status': status,
