...
class EmailAddessManager(models.Manager):
    ...
    def check_email_exist_and_verified(self, user, email, email_address=None):
        status = False
        code = settings.EXO_ACCOUNTS_VALIDATION_CHOICES_NOT_OWNER
    ...
    def check_email(self, user, email):
        ...
        if not user.email == email:
            ...
            if emails.exists():
                ...
                status = False
                code = settings.EXO_ACCOUNTS_VALIDATION_CHOICES_OTHER_USER
    ...
    def change_user_email(self, user_from, user, email):
        ...
        if status:
            if code == settings.EXO_ACCOUNTS_VALIDATION_CHOICES_VERIFIED:
                user.email = email
                user.save()
            else:
                if user_from == user:
    ...
    def exo_account(self):
        try:
            return self.get_queryset().filter(
                type_email=settings.EXO_ACCOUNTS_CH_EXO_EMAIL)[0]
        except IndexError:
            return None
