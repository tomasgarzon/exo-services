...
class UserProfileImageAPITests(SuperUserTestMixin, DjangoRestFrameworkTestCase):

    def setUp(self):
        self.create_superuser()

    def test_change_image_ok(self):
        consultant = FakeConsultantFactory.create()
        self.assertIsNotNone(consultant.user.profile_picture.name)
        self.client.login(
            username=self.super_user.username,
            password='123456',
        )
        url = reverse('api:profile:change-image', kwargs={'pk': consultant.user.pk})
        data = {}
        ...
        self.assertIsNotNone(consultant.user.profile_picture.name)
        self.assertEqual(
            consultant.user.profile_picture_origin,
            settings.EXO_ACCOUNTS_PROFILE_PICTURE_CH_USER)
        self.assertEqual(
            len(consultant.user._meta.get_field('profile_picture').thumbnails),
            len(response.data)
        )
