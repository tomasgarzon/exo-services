...
gunicorn service.wsgi:application --name $SERVICE_NAME --workers 2 --log-level=DEBUG --log-file=- --bind=unix:/projects/service-exo-website/gunicorn.sock &
...
sed -i s/\$AWS_BUCKET/$AWS_BUCKET/ scripts/devops/nginx.conf
sed -i s/\$SERVICE_NAME/$SERVICE_NAME/ scripts/devops/nginx.conf
sed -i s/\$SOURCE_NAME/$SOURCE_NAME/ scripts/devops/nginx.conf

nginx -c /projects/service-exo-website/scripts/devops/nginx.conf  -g 'pid /tmp/nginx.pid; daemon off;'
