...
class PopulatorAPIView(views.APIView):
    ...
    def post(self, request, format=None):
        """
        curl -X POST -H "Content-Type: application/octet-stream" -H \
        "Content-Disposition:inline;filename=salida.json" --data-binary \
        @salida.json  http://localhost:8002/backup/
        """
        content = request.FILES.get('file').read()
        temp_file = tempfile.mktemp(suffix='.json')
        with open(temp_file, 'a') as dump_fd:
