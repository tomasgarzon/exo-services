...
from ..helper import data_job_for_participant
...
class JobCreate(JobMixin, celery.Task):
    name = 'JobCreate'
    ...
    def run(self, *args, **kwargs):
        ...
        url = self.get_url()
        data = data_job_for_participant(job.participant)
...
class JobUpdate(JobMixin, celery.Task):
    name = 'JobUpdate'
...
class JobDelete(JobMixin, celery.Task):
    name = 'JobDelete'
...
class EventOpportunityJobUpdate(celery.Task):
    name = 'EventOpportunityJobUpdate'
    host = settings.EXOLEVER_HOST + settings.SERVICE_JOBS_HOST
    headers = {'USERNAME': settings.AUTH_SECRET_KEY}
    ...
    def run(self, *args, **kwargs):
        if settings.POPULATOR_MODE:
            return
        job = self.get_job(*args, **kwargs)
        opportunity_linked = job.participant.opportunity_related.opportunity_uuid

        url = self.get_url(opportunity_linked, job.participant.user.uuid.__str__())
        try:
