...
class Answer(
    ...
    def can_rate(self, user_from, raise_exceptions=True):
        ...
        if self.post.qa_session:
            author_is_advisor = self.post.qa_session.advisors.filter(
                consultant__user=self.created_by).exists()
            advisor_in_qa = self.post.qa_session.advisors.filter(
                consultant__user=user_from).exists()
